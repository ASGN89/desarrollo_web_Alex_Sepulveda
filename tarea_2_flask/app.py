...
@app.route("/avisos/<int:aviso_id>")
def detalle(aviso_id: int):
    """Detalle de un aviso."""
...
def estadisticas():
    """Vista simple de estadísticas (totales por tipo y conteo diario últimos 14 días)."""
    # Totales por tipo
    tot_perro = db.session.query(db.func.count()).select_from(AvisoAdopcion).filter_by(tipo="perro").scalar() or 0
    tot_gato = db.session.query(db.func.count()).select_from(AvisoAdopcion).filter_by(tipo="gato").scalar() or 0
    ...
    for d in dias:
        cnt = (db.session.query(db.func.count())
               .select_from(AvisoAdopcion)
               .filter(db.func.date(AvisoAdopcion.fecha_ingreso) == d)
               .scalar() or 0)
        por_dia.append((d.isoformat(), cnt))

    return render_template("estadisticas.html",
                           totales={"perro": tot_perro, "gato": tot_gato},
                           por_dia=por_dia)
